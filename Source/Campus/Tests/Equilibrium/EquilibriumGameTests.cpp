...
bool FEquilibriumGameTests::RunTest(const FString& Parameters)
{
	...
	TestTrue("Expected that you can't add here weight {}", !EquilibriumScale.TryAddWeight({}, 5));
	TestTrue("Expected that you can't add here weight { 0 }", !EquilibriumScale.TryAddWeight({ 0 }, 5));
	TestTrue("Expected that you can't add here weight { 1 }", !EquilibriumScale.TryAddWeight({ 1 }, 5));
	TestTrue("Expected that you can't add here weight { 0, 0 }", !EquilibriumScale.TryAddWeight({ 0, 0 }, 5));
	TestTrue("Expected that you can't add here weight { 1, 1 }", !EquilibriumScale.TryAddWeight({ 1, 1 }, 5));
	TestTrue("Expected that you can't add here weight { 0, 1, 0}", !EquilibriumScale.TryAddWeight({ 0, 1, 0}, 5));
	TestTrue("Expected that you can't add here weight { 0, 1, 1 }", !EquilibriumScale.TryAddWeight({ 0, 1, 1 }, 5));
	TestTrue("Expected that you can't add here weight { 1, 0, 0 }", !EquilibriumScale.TryAddWeight({ 1, 0, 0 }, 5));
	TestTrue("Expected that you can't add here weight { 1, 0, 1 }", !EquilibriumScale.TryAddWeight({ 1, 0, 1 }, 5));

	AddInfo("Checking for possible additions");
	for (auto Cup : Cups)
	{
		TestTrue("Expected that you can add here weight ", EquilibriumScale.TryAddWeight(Cup, 2));
		if (Cup != Cups.back())
		{
	...
	for (auto Cup : Cups)
	{
		TestTrue("Expected that you can remove here weight ", EquilibriumScale.TryRemoveWeight(Cup));
		if (Cup != Cups.front())
		{
			TestTrue("Expected that all cups aren't fill", !EquilibriumScale.GetEquilibriumInstance().EveryCupHasWeight());
		}
	}
	TestTrue("Expected that you can remove weight", EquilibriumScale.TryRemoveWeight({1, 1, 1}));

	AddInfo("Checking for possible additions (the second try)");
	for (auto Cup : Cups)
	{
		TestTrue("Expected that you can add here weight ", EquilibriumScale.TryAddWeight(Cup, 5));
	}
	AddInfo("Checking for possible additions to the occupied cups");
	TestTrue("Expected that you can add here weight ", EquilibriumScale.TryAddWeight({ 0, 1 }, 10));
	TestTrue("Expected that you can add here weight ", EquilibriumScale.TryAddWeight( { 1, 0 }, 10));
